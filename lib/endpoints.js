...
function checkAdmin(req, res, next) {
  if (!config.adminPass) return res.staus(500).send('Admin page not set up');
...
app.get('/admin/data.json', checkAdmin, (req, res, next) => {
...
app.delete('/admin/:sid', checkAdmin, (req, res) => {
  const bucket = db.get(req.params.sid)
  let promises = [];
  for (let f of bucket) {
    promises.push(db.remove(req.params.sid, f.key));
  }
...
app.get('/download/:sid', (req, res, next) => {
  if (req.url.endsWith('.json')) {
    ...
    res.json({
      items: db.get(sid).map(data => {
        const item = Object.assign(data, { url: `/download/files/${ sid }++${ data.key }` });
...
// Download files
app.get('/download/files/:fid', async (req, res, next) => {
...
app.use((req, res, next) => {
  res.status(404).send(errorPage.replace('%%ERROR%%', 'Site not found'));
});
